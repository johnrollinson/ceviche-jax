...
def get_entries_indices(csr_matrix):
    # takes sparse matrix and returns the entries and indices in form compatible with 'make_sparse'
...
def make_IO_matrices(indices, N):
    ...
    I = make_sparse(
        entries_1, indices_I, shape=(N, M)
    )  # construct the I matrix
    J = make_sparse(
        entries_1, indices_J, shape=(N, M)
...
def make_rand_indices(N, M):
    # make M random indices into an NxN matrix
    return npa.random.randint(low=0, high=N, size=(2, M))


def make_rand_entries_indices(N, M):
    # make M random indices and corresponding entries
    entries = make_rand_complex(M)
    indices = make_rand_indices(N, M)
    return entries, indices
...
def make_rand_sparse_density(N, density=1):
    ...
    return sp.random(N, N, density=density) + 1j * sp.random(
        N, N, density=density
    )


""" ==================== NUMERICAL DERIVATVES ============================== """
...
def grid_xyz_to_center(Q_xx, Q_yy, Q_zz):
    """Computes the interpolated value of the quantitys Q_xx, Q_yy, Q_zz at
    the center of Yee latice
        Returns these three components
    """
...
def reshape_to_ND(arr, N):
    ...
    if ND > N:
        raise ValueError(
            "array is larger than {} dimensional, given shape {}".format(
                N, arr.shape
            )
        )
    extra_dims = (N - ND) * (1,)
